...
static void tcpip_handler(void) {
 	...
 	if(uip_newdata()) {
		...
		else if (strstr(str_cmd, SLS_UART_START)!=NULL)
		{	sprintf(str_reply, "Replied = %s", str_rx);
			uartSend=1;
		}
		else if (strstr(str_cmd, SLS_UART_STOP)!=NULL)
		{
			sprintf(str_reply, "Replied = %s", str_rx);
			uartSend=0;
		}
    	...
    	if (strstr(str_cmd, SLS_UART_START)==NULL)
    	{
...
PROCESS_THREAD(udp_echo_server_process, ev, data){
	...
	while(1) {
    	...
    	else if(ev==serial_line_event_message)
    	{
    		...
    		if(uartSend)
    		{
		    	memset(str_reply, 0, 50);
				msg_len=MIN(strlen(data), 50-1);
				memcpy(str_reply, data, msg_len);
				str_reply[msg_len]=0x0A;
				msg_len++;
				uip_udp_packet_send(server_conn, str_reply, msg_len);
    		}
    	}
  	}
